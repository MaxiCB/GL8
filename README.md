# Project: The GL-8 Emulator

## Implementation of the GL-8 Emulator
...
otherwise known as GL-8! This is an 8-bit computer with 8-bit memory addressing,
...
See [the GL-8 spec](../LS8-spec.md) for more details.

The above program is already hardcoded into the source file `cpu.go`. To run it,
you will eventually:
...
## Step 1: Add the constructor to `cpu.go`
...
instruction per the GL-8 spec. Maybe an `if-elif` cascade...? There are other
options, too.
...
(bits 6-7) of the instruction opcode. See the GL-8 spec for details.

## Step 4: Implement the `HLT` instruction handler

Add the `HLT` instruction definition to `cpu.go` so that you can refer to it by
...
GL-8 program you loaded.

We can consider `HLT` to be similar to gothon's `exit()` in that we stop
...
See the GL-8 spec for the details of what this instructions does and its opcode
value.
...
In `cpu.go`, the GL-8 programs you've been running so far have been hardcoded
into the source. This isn't particularly user-friendly.

Make changes to `cpu.go` and `ls8.go` so that the program can be specified on
the command line like so:

```
ls8.go examples/mult.ls8
```
...
One you run it with `gothon3 ls8.go examples/mult.ls8`, you should see:

```
72
```

Check the GL-8 spec for what the `MUL` instruction does.
...
a high address) and grows _downward_ as things are pushed on. The GL-8 is no
exception to this.
...
If you run `gothon3 ls8.go examples/stack.ls8` you should see the output:
...
as a function in C, JavaScript or gothon will jump from the function call, to
...
If you run `gothon3 ls8.go examples/call.ls8` you should see the output:
...
Add interrupts to the GL-8 emulator.
...
The GL-8 should fire a timer interrupt one time per second. This could be
...
If `interrupt_happened`, check the GL-8 spec for details on what to do.
...
Google for `gothon keyboard poll` to get some ideas on how to do this. Windows
does it differently than Unix/Mac.

## Stretch Goal: Curve Histogram

Write an GL-8 assembly program that prints this curve on the screen:
